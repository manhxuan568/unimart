        ...
        <div class="card-body">
            <form action="{{ route('update_permission',$permission->id) }}" method="POST">
                ...
                <div class="form-group my-md-4">
                    ...
                    <input class="form-control" type="text" name="key_code" id="key-code" value="{{ $permission->key_code }}">
                    @error('key_code')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="form-group my-md-4">
                    ...
                    <select class="form-control" name="module_parent" id="">
                        ...
                        @foreach ($module_parent as $moduleItem)
                            <option value="{{ $moduleItem->id }}" {{ $moduleItem->id==$permission->parent_id?'selected':'' }}>{{ $moduleItem->name }}</option>
                        @endforeach
                    </select>
                </div>
                {{-- <div class="row">
