                        ...
                        <tbody>
                            ...
                            @endphp                          
                               <tr>
                                ...
                                <td>@if ($cat->status==0)
                                    <span class="badge badge-primary">{{ status($cat->status) }}</span>
                                    @else
                                    <span class="badge badge-success">{{ status($cat->status) }}</span>  
                                    @endif 
                                </td>
                                <td>
                                    @can('edit-category-post')
                                        <a href="{{ route('edit_catPost', $cat->id) }}" class="btn btn-success btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>                                       
                                    @endcan
                                    @can('delete-category-post')
                                        <a href="{{ route('delete_catPost', $cat->id) }}" class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Delete" onclick="return confirm('Bạn có chắc chắn muốn xía menu này?')"><i class="fa fa-trash"></i></a>                                       
                                    @endcan
                                </td>
                            </tr> 
                            @endforeach
